...
export async function buildHtml(config: ThaliaConfig): Promise<void> {
  ...
  await run(['node', insert2html, replacedHtml, modListPath, beforeSc2], { cwd: cacheDir });
...
async function findSingleSourceHtml(pattern: string): Promise<string> {
  ...
  if (!existsSync(dir)) throw new Error(`HTML 输入目录不存在：${dir}`);
  const files = await readdir(dir);
  const htmlFiles = files.filter(file => extname(file).toLowerCase() === '.html').sort();
  if (htmlFiles.length === 0) throw new Error(`HTML 输入目录中没有 .html 文件：${dir}`);
  if (htmlFiles.length > 1) throw new Error(`HTML 输入目录中只能放一个 .html 文件：${dir}`);
...
async function writeModList(modsDir: string, modListPath: string): Promise<void> {
  ...
  const modList = modFiles.map(file => {
    ...
    return relative(dirname(modListPath), target).replaceAll('\\', '/');
  });
